...
select a.ptno
        ...
        select a.*
          ...
          from (
                select /*+ index(a 3C15332B3C20431528_i04) */
                 ...
                 where a.ordr_ymd between to_date('20080101','yyyy-mm-dd') and to_date('20201231','yyyymmdd')
 ...
 where a.fst_ordr_ymd between to_date('20080101','yyyy-mm-dd') and to_date('20160701','yyyymmdd')
   and x.ptno = a.ptno 
   ...
   /* 외국인 제외 */
   and substr(a.brrn,1,1) not in ('5','6','7','8')
...
--                                               ,'1'
 ...
 where a.fst_ordr_ymd between to_date('20170101','yyyy-mm-dd') and to_date('20171231','yyyymmdd')
   and f.ptno = a.ptno 
...
select a.ptno
  ...
  from (-- 기본대상
        select a.*
             ...
             , 펑션.나이계산펑션(a.ptno,a.fst_ordr_ymd) fst_age
             , (
                select min(to_char(y.mdcr_ymd,'yyyy-mm-dd'))
                  from 스키마.3C155B0E153F0E2328 x
 ...
 where a.fst_ordr_ymd between to_date('20090101','yyyy-mm-dd') and to_date('20091231','yyyymmdd')
   and a.ptno not in (/* 기본 제외대상자 */
                      &not_in_ptno
                     )
   /* 동의여부자만 대상 */
