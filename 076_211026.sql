-- rkdalfk
-- DR.H RESULT
with temp as (-- 안압 측정한 사람
              select 
                     a.ptno, a.ordr_ymd
                from 스키마.3E3C0E433E3C0E3E28 a
                   , 스키마.3E3C3C5B0C233C3E28 b
                   , 스키마.3C15332B3C20431528 c
               where a.ordr_ymd between to_date('20071101','yyyymmdd') and to_date('20160701','yyyymmdd')
...
with temp as (-- 안압 측정한 사람
              select 
                ...
                from 스키마.3E3C0E433E3C0E3E28 a
                   , 스키마.3E3C3C5B0C233C3E28 b
                   , 스키마.3C15332B3C20431528 c
               where a.ordr_ymd between to_date('20160702','yyyymmdd') and to_date('20201231','yyyymmdd')
                 and a.cncl_dt is null
                    /* 정보 동의여부자만 대상 */
                 and 펑션.정보제공동의여부펑션(3,a.ptno) = 'Y'
                 and b.pckg_cd = a.pckg_cd
...
--     , 스키마.3E3C3C5B0C233C3E28 d
 where -- a.ptno = '10345585' and
--   and c.ptno = a.ptno
--   and c.ordr_prrn_ymd between to_date('20071101','yyyymmdd') and to_date('20201231','yyyymmdd')
...
-- DR.H QUESAT, 다윈이후 ETL 결과는 배치 DB에서 작업함.
select b.ptno
     , '' cdw_id
     , to_char(b.ordr_prrn_ymd,'yyyy-mm-dd') sm_date
     , b.SMK_YS
     , b.SMK
     , b.SMK_DURATION
     , b.SMK_CURRENT_AMOUNT
     ...
     , b.STRESS_Q20
     , b.STRESS_Q21
  from 스키마.1543294D47144D302E333E0E28 b
 where b.ordr_prrn_ymd between to_date('20160702','yyyymmdd') and to_date('20201231','yyyymmdd')
   and b.ptno not in (
                      &not_in_ptno
                     )
